...
class ProductGateway
{
    ...
    public function getAll():array
    {
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
            $row["is_available"] = (bool) $row["is_available"]; //change this row to boolean
            $data[] = $row;
    ...
    public function get(string $id){
        ...
        return $data;
       //if data doesnt exist returns false
    }
    public function update(array $current, array $new): int
    { //update the record and return number of updates rows
        $sql = "UPDATE product
        SET name = :name, size=:size, is_available = :is_available
        WHERE id = :id"; //write query

        $stmt = $this->conn->prepare($sql); //prepar the statemnt

        //binding variables with placeholder in the query
    ...
    public function delete(string $id):int{
        ...
        $stmt->bindValue(":id", $id, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->rowCount(); //affected number of rows
    }
}